...
請先執行 `案例A_Online_Retail_RFM/02數據分析/rfm_analysis_online_retail.py` 產出上述 CSV，再啟動本儀表板。
...
# 執行儀表板（指定案例 A 的 app.py 路徑）
uv run streamlit run "案例A_Online_Retail_RFM/儀錶板系統上線_streamlit/app.py"
```
...
streamlit run "案例A_Online_Retail_RFM/儀錶板系統上線_streamlit/app.py"
```
